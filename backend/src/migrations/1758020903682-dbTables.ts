...
export class DbTables1758020903682 implements MigrationInterface {
    name = 'DbTables1758020903682'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "student_answers" ("id" SERIAL NOT NULL, "userId" integer NOT NULL, "questionId" integer NOT NULL, "testId" integer NOT NULL, "attemptId" integer NOT NULL, "answer" character varying NOT NULL, CONSTRAINT "PK_3059dcb4fe712a2989ecc32eb6a" PRIMARY KEY ("id", "userId", "questionId", "testId", "attemptId"))`);
        ...
        await queryRunner.query(`CREATE TABLE "tests" ("testId" SERIAL NOT NULL, "title" character varying NOT NULL, "subject" character varying NOT NULL, "instruction" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "publishDate" TIMESTAMP NOT NULL, "expiryDate" TIMESTAMP NOT NULL, "duration" integer NOT NULL, "totalMarks" integer NOT NULL, CONSTRAINT "PK_44b51741ba0ac81af1fbce473e1" PRIMARY KEY ("testId"))`);
        await queryRunner.query(`CREATE TABLE "student_tests" ("id" SERIAL NOT NULL, "userId" integer, "testId" integer, CONSTRAINT "PK_7027e0ecb46286d09b5a4a7cf11" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "users" ("userId" SERIAL NOT NULL, "regId" character varying NOT NULL, "firstName" character varying(255) NOT NULL, "lastName" character varying(255) NOT NULL, "class" character varying NOT NULL, "school" character varying NOT NULL, "phoneNo" character varying NOT NULL, "fatherName" character varying NOT NULL, "parentNo" character varying NOT NULL, "address" character varying NOT NULL, "password" character varying NOT NULL, "role" "public"."users_role_enum" NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP, CONSTRAINT "UQ_63f4d7b8ea5ce2392e024ae59c9" UNIQUE ("regId"), CONSTRAINT "PK_8bf09ba754322ab9c22a215c919" PRIMARY KEY ("userId"))`);
        await queryRunner.query(`ALTER TABLE "student_answers" ADD CONSTRAINT "FK_ce2df3c8edc1ae9c823d3d6aea3" FOREIGN KEY ("userId") REFERENCES "users"("userId") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE "student_tests" ADD CONSTRAINT "FK_e2d3e586a436d5b1b8661b5c558" FOREIGN KEY ("userId") REFERENCES "users"("userId") ON DELETE NO ACTION ON UPDATE NO ACTION`);
